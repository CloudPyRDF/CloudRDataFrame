...
ReplicateRange = namedtuple(
    "ReplicateRange", ["start", "length", "source", "destination", "part_number", "id"]
)
MB = 1024**2
...
class Replicator:
    ...
    def replicate_files(self, to_replicate: list[tuple[str, str]]):
        ...
        for rr in first_ranges:
            logger.debug(
                f"RepRange(uid={rr.id}, src={rr.source}, dst={rr.destination})"
            )

        # TODO: Casting ranges to tuples is for serialization:
        # Lambda implementation uses namedtuples, which are not yet present
        # in the base container within our version of DistRDF, so we here cast
        # ReplicateRange to tuple and inside lambda we use the same definition
        # in the code and make ReplicateRange out of tuples.
    ...
    def process_files(self, files: list[str]):
        ...
        for src_filename in files:
            ...
            if parsed.scheme != "s3" and not self.aws.s3_object_exists(
                self.bucket, dst_filename
            ):
    ...
    def create_replicate_ranges(self, to_replicate: list[tuple[str]]):
        ...
        for src_filename, dst_filename in to_replicate:
            file_ranges = self.create_replicate_ranges_batch(src_filename, dst_filename)
    ...
    def create_replicate_ranges_batch(
        ...
        upload_id = self.aws.start_multipart_upload(self.bucket, dst_filename)
    ...
    def shuffle_and_partition(
        self, ranges: list[ReplicateRange], parts_count: int
    ) -> list[tuple[ReplicateRange]]:
        random.shuffle(ranges)
        return self.partition(ranges, parts_count=parts_count)

    @staticmethod
    def partition(iterable, parts_count: int) -> list[tuple]:
        chunk_size, leftover = divmod(len(iterable), parts_count)
        evenly_dividable_count = chunk_size * parts_count
        evenly_dividable = iterable[:evenly_dividable_count]
        chunked = Reducer.divide_into_chunks(evenly_dividable, chunk_size)

        for i in range(leftover):
            chunked[i] += (iterable[evenly_dividable_count + i],)

        return chunked

    def invoke_replicating_lambdas(self, chunks):
        invoke = functools.partial(
            self.aws.invoke_replicate_lambda, certs=self.cert.bytes
        )
    ...
    def get_upload_parts(self, uid: str, destination: str) -> list:
        parts = self.aws.list_all_parts(
            Bucket=self.bucket, Key=destination, UploadId=uid
        )
