...
def run_async(event_time, filename):
    ...
    if ReductionTree.is_left(partial_number) and not ReductionTree.exists(
        twin_number, file_count
    ):
        print("left without twin")
    ...
    if twin_event_time is None:
        print("could not get twin - probably not yet created")
...
def get_args(filename):
    m = re.match(pattern=r"output/([^/]+)/partial_(\d+)_(\d+).out", string=filename)
    if m is None:
