...
"""
    ...
    def add_lambda(self, lambda_code):
        ...
        self._lambdas += "\n  auto rdf_lambda{n} = {code};".format(
            n=self._lambda_id, code=lambda_code
        )
        self._lambda_id += 1
    ...
    def add_node(self, operation, range_id, parent_id):
        ...
        op_call = "rdf{n}.{op}{templ_args}({args});".format(
            n=parent_id,
            op=operation.name,
        ...
        if operation.is_transformation():
            ...
            self._graph_nodes += "\n  auto rdf{n} = {call}".format(
                n=new_node_id, call=op_call
            )
            self._node_id += 1
            return new_node_id

        # Else it's an action or instant action
        self._graph_nodes += "\n  auto res_ptr{n} = {call}".format(
            n=self._res_ptr_id, call=op_call
        )

        # The result is stored in the vector of results to be returned
        self._graph_nodes += "\n  result_handles.emplace_back(res_ptr{});".format(
            self._res_ptr_id
        )
        ...
        self._graph_nodes += (
            ...
            "\n  result_types.emplace_back(c{0}->GetName());".format(
                self._res_ptr_id, operation.name
            )
        )
    ...
    def _handle_snapshot(self, operation, range_id):
        ...
        path_with_range = "{filename}_{rangeid}.root".format(
            filename=filename, rangeid=range_id
        )
    ...
    def _handle_asnumpy(self, operation, parent_id):
        ...
        self._py_actions.append(CppWorkflow._PyActionData(self._res_ptr_id, operation))
        self._res_ptr_id += 1

        # Save parent RDF node to run AsNumpy on it later from Python
        self._graph_nodes += (
            "\n  output_nodes.push_back(ROOT::RDF::AsRNode(rdf{}));".format(parent_id)
        )
    ...
    def _get_call_args(self, operation):
        ...
        for narg, arg in enumerate(operation.args):
            ...
            if isinstance(arg, str):
                ...
                ROOT.Warning(
                    "DistRDF", "Processing of RSnapshotOptions not implemented"
                )
    ...
    def _compile(self):
        ...
        cpp_file_name = "rdfworkflow_{wf_id}_{pid}.cxx".format(
            wf_id=this_wf_id, pid=os.getpid()
        )
        final_code = code.replace(
            CppWorkflow._FUNCTION_NAME, CppWorkflow._FUNCTION_NAME + str(this_wf_id), 1
        )
        ...
        if not ROOT.gSystem.CompileMacro(cpp_file_name, "O"):
            raise RuntimeError(
                "Error compiling the RDataFrame workflow file: {}".format(cpp_file_name)
            )
