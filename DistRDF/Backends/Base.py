...
class BaseBackend(ABC):
    ...
    def check_supported(self, operation_name):
        ...
        if operation_name not in self.supported_operations:
            raise Exception(
                'The current backend doesn\'t support "{}" !'.format(operation_name)
            )
    ...
    def execute(self, generator):
        ...
        def mapper(current_range):
            ...
            if optimized:
                ...
                mergeables = [
                    Utils.get_mergeablevalue(resultptr) for resultptr in resultptr_list
                ]
    ...
    def distribute_files(self, files_paths):
        ...
        if isinstance(files_paths, str):
            files_to_distribute.update(Utils.get_paths_set_from_string(files_paths))
        else:
            for path_string in files_paths:
                files_to_distribute.update(Utils.get_paths_set_from_string(path_string))
    ...
    def distribute_headers(self, headers_paths):
        ...
        if isinstance(headers_paths, str):
            headers_to_distribute.update(Utils.get_paths_set_from_string(headers_paths))
        else:
