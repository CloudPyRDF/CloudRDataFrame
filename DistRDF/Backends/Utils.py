...
def get_paths_set_from_string(path_string):
    ...
    if os.path.isdir(path_string):
        ...
        paths_set = {
            ...
            for rootpath, dirs, filenames in os.walk(path_string)
            for filename in filenames
        }
        logger.debug(
            "\nInitial path: {} \nPaths retrieved: {}".format(path_string, paths_set)
        )
        return paths_set
...
def check_pcm_in_library_path(shared_library_path):
    ...
    """
    all_paths = get_paths_set_from_string(shared_library_path)

    pcm_paths = {filepath for filepath in all_paths if filepath.endswith(".pcm")}

    shared_library_formats = (".so", ".dll", ".dylib")
    libraries_path = {
        filepath for filepath in all_paths if filepath.endswith(shared_library_formats)
    }
