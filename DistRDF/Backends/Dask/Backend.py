...
try:
    import dask
    from dask.distributed import Client, LocalCluster, get_worker, progress
except ImportError:
    raise ImportError(
        (
            "cannot import a Dask component. Refer to the Dask documentation "
    ...
    def __init__(self, daskclient=None):
        ...
        self.client = (
            ...
            if daskclient is not None
                ...
                LocalCluster(
                    n_workers=os.cpu_count(), threads_per_worker=1, processes=True
                )
            )
        )
