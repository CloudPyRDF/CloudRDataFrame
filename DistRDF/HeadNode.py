...
from DistRDF import Node, Ranges
    ...
    if isinstance(firstarg, int):
        ...
        raise RuntimeError(
            (
                ...
                "can be processed distributedly."
            ).format(firstarg, type(firstarg))
        )
    ...
    def build_ranges(self):
        ...
        if self.npartitions > self.nentries:
            ...
            msg = (
                ...
                "in the dataframe. Using {1} partition(s)".format(
...
class TreeHeadNode(Node.Node):
    ...
    def __init__(self, npartitions, *args):
        ...
        self.subtreenames = [
            str(treename)
            for treename in ROOT.Internal.TreeUtils.GetTreeFullPaths(self.tree)
        ]
        self.inputfiles = [
            str(filename)
            for filename in ROOT.Internal.TreeUtils.GetFileNamesFromTree(self.tree)
        ]
    ...
    def build_ranges(self):
        ...
        if self.npartitions > numclusters:
            msg = (
                "Number of partitions is greater than number of clusters "
        ...
        logger.debug(
            "%s clusters will be split along %s partitions.",
            numclusters,
            self.npartitions,
        )
        return Ranges.get_clustered_ranges(
            clustersinfiles, self.npartitions, self.friendinfo
        )
    ...
    def generate_rdf_creator(self):
        ...
        def build_rdf_from_range(current_range):
            ...
            for start, end, filename, treenentries, subtreename in zip(
                current_range.localstarts,
                current_range.localends,
                current_range.filelist,
                current_range.treesnentries,
                current_range.treenames,
            ):
            ...
            if (
                current_range.friendinfo is not None
                ...
                for (
                    ...
                    if friend_chainsubnames.empty():
                        ...
                        for filename, chainsubname in zip(
                    ...
                    friend_chain.SetCacheEntryRange(
